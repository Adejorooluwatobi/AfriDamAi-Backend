...
import { UserSubscription as PrismaSubscription, PricingPlan as PrismaPricingPlan } from '@prisma/client'; // Import Prisma types
...
export interface ISubscriptionRepository {
  ...
  findByIdWithPlan(id: string): Promise<(PrismaSubscription & { pricingPlan: PrismaPricingPlan }) | null>; // Modified return type
  findByUserId(userId: string): Promise<UserSubscriptionEntity[]>;
  findByUserIdWithPlan(userId: string): Promise<(PrismaSubscription & { pricingPlan: PrismaPricingPlan })[]>;
  ...
  findUsersByStatus(status: string): Promise<any[]>; // Returns users with their subscription details
}
