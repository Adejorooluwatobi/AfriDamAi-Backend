"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.PrismaWishlistRepository = void 0;
const common_1 = require("@nestjs/common");
const wishlist_entity_1 = require("../../../domain/entities/wishlist.entity");
const prisma_service_1 = require("./prisma.service");
const wishlist_item_mapper_1 = require("../../mappers/wishlist-item.mapper");
let PrismaWishlistRepository = class PrismaWishlistRepository {
    constructor(prisma) {
        this.prisma = prisma;
    }
    mapToDomain(wishlist) {
        const items = (wishlist.items || []).map((i) => wishlist_item_mapper_1.WishlistItemMapper.toDomain(i));
        return new wishlist_entity_1.WishlistEntity({
            id: wishlist.id,
            userId: wishlist.userId,
            createdAt: wishlist.createdAt,
            updatedAt: wishlist.updatedAt,
            items,
        });
    }
    async create(params) {
        const wishlist = await this.prisma.wishlist.create({
            data: params,
            include: { items: true },
        });
        return this.mapToDomain(wishlist);
    }
    async findByUserId(userId) {
        const wishlist = await this.prisma.wishlist.findUnique({
            where: { userId },
            include: {
                items: {
                    include: { product: true }
                }
            },
        });
        if (!wishlist)
            return null;
        return this.mapToDomain(wishlist);
    }
    async findById(id) {
        const wishlist = await this.prisma.wishlist.findUnique({
            where: { id },
            include: {
                items: {
                    include: { product: true }
                }
            },
        });
        if (!wishlist)
            return null;
        return this.mapToDomain(wishlist);
    }
    async addItem(params) {
        await this.prisma.wishlistItem.create({ data: params });
        const wishlist = await this.prisma.wishlist.findUnique({
            where: { id: params.wishlistId },
            include: {
                items: {
                    include: { product: true }
                }
            },
        });
        if (!wishlist)
            throw new common_1.NotFoundException('Wishlist not found');
        return this.mapToDomain(wishlist);
    }
    async removeItem(params) {
        await this.prisma.wishlistItem.deleteMany({
            where: { wishlistId: params.wishlistId, productId: params.productId },
        });
        const wishlist = await this.prisma.wishlist.findUnique({
            where: { id: params.wishlistId },
            include: {
                items: {
                    include: { product: true }
                }
            },
        });
        if (!wishlist)
            throw new common_1.NotFoundException('Wishlist not found');
        return this.mapToDomain(wishlist);
    }
    async clear(wishlistId) {
        await this.prisma.wishlistItem.deleteMany({
            where: { wishlistId },
        });
    }
};
exports.PrismaWishlistRepository = PrismaWishlistRepository;
exports.PrismaWishlistRepository = PrismaWishlistRepository = __decorate([
    (0, common_1.Injectable)(),
    __metadata("design:paramtypes", [prisma_service_1.PrismaService])
], PrismaWishlistRepository);
//# sourceMappingURL=prisma-wishlist.repository.js.map